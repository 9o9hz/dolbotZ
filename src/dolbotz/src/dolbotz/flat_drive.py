...
캘리브레이션 피클을 지정하면 그 값이 이 상수보다 우선 적용된다):
  ...
  model_path                str    기본값 dolbotz.utils.paths.get_models_dir()/'dolbotz_seg_v1'/'best_openvino_model'
...
class FlatDriveNode(Node):
    ...
    def __init__(self):
        ...
        # 기본값으로 쓴다 (resolve_mount_defaults 참고, elevation_map.py와 동일
        # 파라미터명 — 같은 카메라이므로 launch에서 값 공유 가능). --ros-args로
        # 명시적으로 넘긴 값은 이 기본값과 무관하게 항상 최우선 적용된다.
        ...
        self.declare_parameter('model_path', str(get_models_dir() / 'dolbotz_seg_v1' / 'best_openvino_model'))
    ...
    def _load_model(self, path: str):
        ...
        if not _YOLO_OK:
            ...
            return None
        # task='segment'를 명시한다 — ultralytics는 OpenVINO IR 모델의 task를
        # metadata.yaml이 아니라 경로 문자열에 'segment'라는 디렉토리 이름이
        # 포함되는지로 추측한다(nn/tasks.py의 guess_model_task). 원래
        # runs/segment/.../best_openvino_model 경로는 우연히 이 휴리스틱을
        # 통과했지만, config/models/dolbotz_seg_v1/best_openvino_model로 옮긴
        # 뒤에는 'segment'라는 경로 조각이 없어 task=detect로 잘못 추측되어
        # r.masks가 항상 None이 되는 회귀가 있었다 — 그런 경로 이름 우연에
        # 의존하지 않도록 task를 명시적으로 고정한다.
